  ...
  try {
    ...
    if (invoice.qrCode) {
      try {
        ...
        console.log('📄 QR Code Data URL:', invoice.qrCode.substring(0, 100) + '...');
        
        // Decode QR code data to verify content
        try {
          const qrDataString = invoice.qrCode.split(',')[1]; // Remove data:image/png;base64, prefix
          const decodedData = atob(qrDataString);
          console.log('🔍 Decoded QR data preview:', decodedData.substring(0, 200));
        } catch (decodeError) {
          console.log('⚠️ Could not decode QR data for preview');
        }
