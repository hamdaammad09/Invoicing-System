...
const generatePDFInvoice = async (invoice, buyer, seller) => {
  try {
    ...
    if (invoice.qrCode) {
      try {
        console.log('🔄 Adding QR code to PDF (bottom left)...');
        
        // Create image from QR code data URL
        const qrImage = new Image();
        qrImage.src = invoice.qrCode;
        
        // Position QR code at the bottom left
        const qrSize = 45; // Size of QR code
        const qrX = 20; // X position (left side)
        ...
        console.log('✅ QR code added successfully at bottom left');
